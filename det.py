...
# STEP 1: Import the necessary modules. # 모듈 가져오기
import numpy as np
import mediapipe as mp
from mediapipe.tasks import python
from mediapipe.tasks.python import vision

# STEP 2: Create an ObjectDetector object. 추론기 : 모델정보 넣어서 추론기 만들기
base_options = python.BaseOptions(model_asset_path='models\\efficientdet_lite0.tflite')
options = vision.ObjectDetectorOptions(base_options=base_options,  #모델옵션
                                       score_threshold=0.5)
detector = vision.ObjectDetector.create_from_options(options) # 추론기 만들어주는 함수
...
# STEP 5: Process the detection result. In this case, visualize it. #후처리: 사람한테 어떻게 잘 보여줄것인가 시각화
